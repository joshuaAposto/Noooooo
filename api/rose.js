...
const meta = {
  name: "rose",
  version: "1.0.0",
  description: "Your vampire roommate enters your room at night, with a request",
...
async function onStart({ req, res }) {
  try {
    ...
    if (!prompt) {
      return res.status(400).json({ error: "Please add ?prompt=your_message_here" });
    }

    const data = {
      character_id: "1734336114074148336",
      content: prompt
    };
    ...
    res.status(500).json({ 
      error: "Failed to communicate with character ai"
    });
  }
}
